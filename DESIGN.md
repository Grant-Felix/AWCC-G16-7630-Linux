...
窗口按 1280×800 逻辑像素做（截图是 2560×1600，2 倍；所以测得像素除以 2 即逻辑值）。
**下面这些数字是用 ImageMagick 沿行/列采样、找颜色突变处量出来的**，不是估的：

| 项 | 实测值（逻辑 px） | 备注 |
| --- | --- | --- |
| 标题栏高度 | 48 | 面包屑在左侧；右侧红色胶囊按钮也在栏内；辉光从 y=48 开始 |
| 左栏宽度 | ~48 | 无独立底色；选中项是红色圆角方块（约 32×32，圆角 8） |
| 模式按钮 | 高 35，宽 70~80，间距 20 | 纯文字、无卡片、整行居中，位于 y≈100..135 |
| 内容卡起始 | y≈172 | 辉光渐隐之后 |
| 卡片圆角 | ~8 | 面板色 #0F1521、边框 #1B1F24 |
| 环形表 | 外径 ≈130，描边 ≈8 | 灰轨道 #3A3F47 + 从 12 点顺时针红弧 #FD5643 |
| 参数表行高 | ~40 | 左灰标签 + 右白数值 + 1px 分割线 #1B1F24 |
| 底部段控 | y≈740..780，居中 | 「概况 / 散热」这类，选中为红块 |

**一处要说清的限制**：参考图 `_4_9` 里第 4 列（C 盘概况）被裁掉了一部分，所以列宽之类的
**绝对横向坐标不可靠**；上表只收了能可靠量到的值，横向按「等宽四列 + 32px 间距 + 内容区居中」
反推。

各页构成（读自 14 张参考图，元素一律做成圆角卡片）：

| 页面 | 构成 |
| --- | --- |
| 主页 `_3_9` | 顶部模式行（居中）→ 中部整宽大卡（机身图）→ 底部一行两卡：左 4 个环形仪表卡（占约 2/3）＋右游戏库卡（约 1/3） |
| 性能 · 概况 `_4_9` | 模式行 → 整宽卡内四等列（每列：小图标＋灰标题 / 环形表 / 参数表）→ 底部段控「概况 / 散热」＋右下「性能设置」 |
| 性能 · 散热 `_6_9` | 模式行 → 卡内 2 个温度环＋2 个风扇转速环 → 底部段控 |
| ALIENFX™ `_7_9` | 左大卡（整块键盘可视化，约 2/3）＋右卡（「灯效 / 按键绑定」两个 tab、预设与效果下拉、亮度与持续时间、取消 / 保存预设） |
| 设置 `_9_9` | 左卡（二级列表：关于 / 外观 / 覆盖 / 新手入门 / 全局预设 / 性能）＋右卡（详情） |
| 库 `_16_9` | 顶部工具行（搜索 / 刷新 / 添加游戏 / 标签页 / 排序 / 缩放）＋网格卡 |

「所有元素以圆角卡片形式存在」是明确要求：官方截图里面板与底色只差两级（#0F1521 vs #11151D），
几乎看不出边界；本 fork 在此按需求把面板做成**看得出的圆角卡片**（保留官方配色，加强边框与
圆角对比）。
...
**判断标准只有一条：谁能改这个值。** 电源模式的写者至少有四个——GUI 点按钮、键盘上的 F9（走
daemon 的按键监听）、将来的桌面环境快捷键、命令行工具。写者不止一个，**真相就只能放在最底层**
（daemon 与它背后的硬件）；GUI 手里那份只是「影子」。

由此定下四条约定：

- **daemon 持有状态**（模式、亮度、按键绑定版本），任何改动都经过它；
- **GUI 不持有真相**：只保存「最近一次从后端收到的快照」（`AppContext` 就是那个 store），所有
  控件从 store 渲染，而不是建页时各自读一次——议题 #1「主页改了、性能页不动」就是这么来的；
- **变更走同一条路**：后端改 → 后端通知 → 前端重画；
- **GUI 自己 set 时不做乐观更新**：只把命令发出去，等后端通知回来再渲染，避免出现「界面显示 A、
  硬件实际是 B」。

通知机制用**套接字订阅推送**，不用轮询、也不上 D-Bus：

- GUI 连上后发 `subscribe`，daemon 先推一份当前快照，之后**只在值真的变了**才推一行
  `<属性> <值>`（`mode performance`、`brightness 50`、`keybinds 3`）；
- 不用 D-Bus 的原因：属性 + `PropertiesChanged` 更「桌面正统」，但要给 root 的 daemon 配 system
  bus 服务与权限策略；本工程已经有套接字，加一条命令的成本小得多、也不动权限模型。将来要对外
  集成时，再把同一份状态挂一份到 D-Bus 上；
- 不用轮询的原因：1 秒延迟能接受，但空闲时也要一直问；订阅是「该动才动」。

本版明确**未**纳入订阅的部分（避免误以为已同步）：CPU / GPU boost 仍是各自 set / read 的请求-应答；
通过原始 ACPI 命令路径（白名单 + popen）改的东西 daemon 不追踪——GUI 侧的模式与亮度改走
`mode-set` / `brightness-set` 这两条结构化命令，所以日常路径都在追踪范围内。
