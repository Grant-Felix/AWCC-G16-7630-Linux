...
## v26.9.23-1 (2026-09-23)

### ⌨️ 按键绑定（议题 #2）

- 绑定表改成配置驱动：`/etc/awcc/keybinds.conf`，主键是**实测的 EV_MSC 扫描码**（G 模式键 104、
  F2～F6 = 146～150；其中 F4/F6 只发扫描码、没有 EV_KEY），出厂默认 G 模式键切 G 模式、灯键循环亮度
- **ALIENFX · 按键绑定**成为真页面：列出实测键位、下拉选动作（不绑定 / 切换 G 模式 / 循环亮度 /
  六档模式）、保存结果就地提示；daemon 没跑时给说明
- 动作可选：`none`、`gmode-toggle`、`brightness-cycle`、`mode:{battery,cool,quiet,balanced,performance,gmode}`

### 🔄 状态所有权与订阅（DESIGN 第九节）

- 真相定在 daemon：GUI 只保存「最近一次从后端收到的快照」，控件从它渲染，set 时只发命令、
  **不做乐观更新**
- 套接字新增 `subscribe`（先推快照，之后只在值真的变了才推一行）、`mode-set`、`brightness-set`；
  三条都在我们这层解析，不走白名单 + popen
- 因此**按 F9 改模式时 GUI 会自己跟上**——议题 #1 留下的那个隐患消失

### 🛠 安装方式改回源码脚本

- **不再维护 deb / rpm / AUR 配方**（那三套要同时跟进，收益不如成本），改成 `install.sh`：
  交互式 TUI，检查依赖 → 本机构建 → 安装到系统，并把**文件清单**记到
  `/var/lib/awcc/installed-files.list`，卸载按清单逐个删除（不猜哪些文件可能是我们装的）
- 菜单用数字输入（本机终端对方向键支持不佳），安装/卸载/只构建/只检查依赖四件事都能单独选
- `AWCC_INSTALL_ROOT` 可把整套装到别处，不动系统（试用与测试用）

### 🐛 修复

- **议题 #1**：主页 / 性能·概况 / 性能·散热 三页的电源模式各自为政（一页改了另外两页不动）。
  现在模式状态只有一份，三页按钮共享并统一刷新；建页时设初始选中改为静默（原来会反过来真写一次
  模式，daemon 没跑时会弹 pkexec 授权框）
- 状态标签原来用 `makeLabel("")`，而 `gettext("")` 按约定返回整个 `.po` 的头部，界面上会渲染出
  po 头；已改成直接建空标签
- 构建：libevdev 外部项目钉到具体提交并关掉「configure 时去上游核对更新」（上游 gitlab 会回 500
  把构建打断，且 `GIT_TAG master` 不可复现）
...
- 前端从上游的 ImGui 换成 **GTK4 + libadwaita**（C API）：外壳用 `GtkBuilder` `.ui` + `GResource`；
  imgui / glfw / OpenGL / X11 / stb 依赖，上游那套 ImGui 界面，以及 15 万行的 `resources.cpp`
  一并退役（其中 5 个模式图标抽成 `assets/modes/*.png` 留用）
- 样式与版式**照官方 AWCC 截图量出来做**：强调红 `#FD5643`、面板 `#0F1521`、未选中按钮 `#0B0B0E`、
  边框与分割线 `#1B1F24`、次要文字 `#AEAFB3`；标题栏 48、左栏 ~48、模式按钮高 35、环形表外径 130、
  描边 8、卡片圆角 8、表格行高 40
- 主页 / 性能·概况 / 性能·散热 已按官方版式改（模式行居中、整宽卡、底部 2/3 + 1/3、卡内四等列、
  风扇转速锯齿环）；ALIENFX / 设置 / 库 三页仍是过渡版式（见「已知欠缺」）
- 既有功能接入：电源模式、7 种灯效、亮度、CPU / GPU 风扇 boost、睿频、设备信息
- **中英文自动切换**（gettext：源串英文 + `po/zh_CN.po`，136 条译文无遗漏）
- 图标随窗口等比缩放；左栏图标按实测墨迹自动补偿，一排视觉大小一致
- 未接通后端的地方一律显示「—」并挂「施工中」徽标，**不显示假数据**

### 🎨 背景可配置

- 默认**纯黑**；另可选跟随系统亮暗、自定义颜色、自定义图片、官方蓝色辉光
- 配置落在 `$XDG_CONFIG_HOME/awcc/config.ini`，「设置 · 外观」页可直接改
- 顺带修掉上游遗留的「亮度每次打开都回到 50%」——根因是它写在 root 所有的 `/etc/awcc/brightness`

### 🛠 工程

- 版本号改为日期式 `v<YY>.<M>.<D>-<序号>`，`scripts/release.sh` 负责算号与打 tag；
  上游那套只会产 semver 的 release-please 已移除
- 新增无人值守模式：`--ui-selftest`（页面树自检）、`--ui-snapshot=<png>`（无显示器也能自渲染截图）、
...
- ALIENFX / 设置 / 库三页的版式还没换成官方那种（ALIENFX 应为左键盘卡 + 右面板，设置应为左列表 + 右详情）
- GPU 遥测受阻：dGPU 处于 runtime suspend 且 `/dev/nvidia*` 不存在，`nvidia-smi` 无法与驱动通信
