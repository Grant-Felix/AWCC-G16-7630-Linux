...
**议题 #1（主页与性能页模式不同步）已修（2026-09-22）**：原来主页 / 性能·概况 / 性能·散热
三页各建一组模式按钮、各自记着建页那一刻的旧值，所以一页改了另外两页不动，任一处点击也读不回
真实结果。现在模式状态只有一份（`AppContext::currentMode`），三页按钮都登记进去，改一处后由
`syncModeButtons()` 统一刷新按钮与「当前模式」文字；建页设初始选中改为静默——原来那一句会反过来
真去写一次模式，daemon 没跑时就会弹 pkexec 授权框。自检里加了回归项：
...
动作先用现成的：`gmode-toggle`、`brightness-cycle`、`mode:{battery,cool,quiet,balanced,performance,gmode}`；
自定义命令要等白名单方案，暂不提供。界面上的「按键绑定」页仍是占位，是下一段。
